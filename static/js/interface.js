...
function sendMotorCommand(command, args = null) {
    ...
    // Show loading state
    updateMotorStatusDisplay(`Executing ${command}...`, 'info');
    ...
    .then(data => {
        if (data.status === "success") {
            console.log(`Motor command ${command} executed successfully:`, data.result);
            updateMotorStatusDisplay(`${command} executed successfully`, 'success');
            return data.result;
        } else {
    ...
    .catch(error => {
        console.error(`Motor command ${command} failed:`, error);
        updateMotorStatusDisplay(`${command} failed: ${error}`, 'error');
...
function getMotorStatus() {
    ...
    .then(result => {
        if (result && typeof result === 'object') {
            let statusText = "<strong>Motor Status:</strong><br>";
            for (const [key, value] of Object.entries(result)) {
                statusText += `<span class="text-muted">${key}:</span> ${value}<br>`;
            }
